<script lang="ts">
        ...
        const generateImage = async (message: MessageType) => {
                ...
                const res = await imageGenerations('static-token', message.content).catch((error) => {
                        toast.error(`${error}`);
                });
        ...
        const feedbackHandler = async (rating: number | null = null, details: object | null = null) => {
                ...
                if (!details) {
                        if (!updatedMessage.annotation?.tags && (message?.content ?? '') !== '') {
                                ...
                                const tags = await generateTags('static-token', message.model, messages, chatId).catch(
                                        (error) => {
                                                console.error(error);
                                                return [];
                                        }
                                        ...
                                        >
                                                {#if message.done || siblings.length > 1}
                                                        ...
                                                        {#if message.done}
                                                                ...
                                                                {#if false && ($user?.role === 'admin' || $user?.permissions?.features?.image_generation) && !readOnly}
