<script lang="ts">
        ...
        export let className = 'min-w-fit';
        ...
        const pullModelHandler = async () => {
                ...
                const [res, controller] = await pullModel('static-token', sanitizedModelTag, '0').catch(
                        (error) => {
                                toast.error(`${error}`);
                                return null;
                        }
        ...
        const setOllamaVersion = async () => {
                ollamaVersion = await getOllamaVersion('static-token').catch((error) => false);
        };
...
>
        ...
        >
                <div
                        class="flex w-auto text-left bg-transparent whitespace-nowrap text-[17px] font-medium justify-between {($settings?.highContrastMode ??
                        false)
                ...
                >
                        ...
                        {:else}
                                {placeholder}
                        {/if}
                        <Icon name="ChevronDown" className=" self-center ml-2 size-3" strokeWidth="2.5" />
                </div>
        </DropdownMenu.Trigger>
